...
export type StakeForResult = {
  hash: Hex;
  amount: bigint;
};

export type StakeForStep =
  | "idle"
  | "checking-allowance"
  | "approving"
  | "staking"
...
export function useStakeFor() {
  ...
  const [step, setStep] = useState<StakeForStep>("idle");
  ...
  >({
    ...
    mutationFn: async ({ predictionId, amount }) => {
      ...
      console.log("✅ Stake FOR transaction submitted:", hash);
      setTxHash(hash);
      setStep("waiting");

      await publicClient.waitForTransactionReceipt({ hash });
      console.log("✅ Stake FOR transaction confirmed");
    ...
    onError: (error: Error) => {
      console.error("❌ Stake FOR error:", error);
      setStep("error");
    },
  });
